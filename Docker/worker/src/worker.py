...
class Worker:
    def __init__(self, base_path: pathlib.Path) -> None:
        ...
        loguru.logger.info("Waiting for clickhouse to start...")
        self.linker = Linker(base_path=base_path)

    @loguru.logger.catch()
    async def run(self) -> None:
        loguru.logger.info("Connected to clickhouse ✅")
